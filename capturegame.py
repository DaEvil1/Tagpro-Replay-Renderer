...
class Game:
    ...
    def __init__(self, filepath, w, h):
        ...
        self.data["folder"] = "/SourceIMG/Game_"
        self.data["raw"] = []
        self.data["last frame"] = 0
        self.ended = False
...
#            print(hour, minute, second, "   "),
    ...
    def SaveFrame(self, win, name):
        path = os.path.dirname(__file__)
        folder = self.data["folder"] + name + "/"
        length = len(str(len(self.data["replay"]["clock"])))
        zeros = "0"*(length - len(str(self.data["frame"])))
        file = zeros + str(self.data["frame"]) + ".bmp"
        ...
        if not exists:
#            print(fullpath)
            win.Modify("save", fullpath)

    def SaveFrame2(self, win, name):
        image = win.Get("screen")
        raw = pygame.image.tostring(image, "RGB")
        print(len(raw))
        self.data["raw"].append(raw)
        length = len(len(self.data["replay"]["x"]))
        if self.data["frame"] < self.data["last frame"]:
            path = os.path.dirname(__file__)
            folder = self.data["folder"] + name + "/"
            zeros = "0"*(length - len(str(self.data["frame"])))
            file = zeros + str(self.data["frame"]) + ".txt"
            ensure_dir(path + folder)
            relpath = folder + file
            with open(relpath, "w") as f:
                for i in self.data["raw"]:
                    f.write(i + "\n")
        self.data["last frame"] = self.data["frame"]
